...
contract Verify {
    function unCompressedPubKey(
        ...
        return
            abi.encode(
                publicKey,
                bytes32(EllipticCurve.deriveY(prefix, uint256(publicKey)))
            );
    }
    ...
    function verifyMessage(
        ...
        bytes memory btcSignature,
        bytes memory btcPublicKey,
        address evmAddress
    ) public pure returns (bool) {
        message = string.concat(message, toString(evmAddress));
        bytes32 btcSignedMessageHash = sha256(
            abi.encodePacked(
                sha256(
                    abi.encodePacked(
                        "\x18Bitcoin Signed Message:\n",
                        bytes1(uint8(bytes(message).length)),
                        message
                    )
                )
            )
        ...
        return
            calculateAddress(unCompressedPubKey(btcPublicKey)) ==
